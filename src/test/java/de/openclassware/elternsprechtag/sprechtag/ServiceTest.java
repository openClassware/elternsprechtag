...
/**
 * Gemeinsame Spring-Test-Konfiguration der Use-Case-Naht.
 *
 ...
 *
 * <p>Sie liegt im Wurzelpaket des Kontexts, weil beide Seiten sie brauchen: die Use-Case-Tests unter
 * {@code application.service} und die Versand-Tests unter {@code adapter.out.mail}, die denselben
 * Aufbau samt Fixtures verlangen.
 *
...
// Der Slice ersetzt die Datasource sonst per Default (Replace.NON_TEST) durch eine
// eingebettete — die Suite bliebe grün, liefe aber gegen eine andere Datenbank als die
// Produktion. Nur so läuft dieselbe Migrationskette auch in Tests.
// Preis: die Use-Case-Tests brauchen eine laufende Datenbank (siehe docs/ci.md).
@AutoConfigureTestDatabase(replace = AutoConfigureTestDatabase.Replace.NONE)
// Flyway gehört nicht zum Umfang des @DataJdbcTest-Schnitts. Ohne diese Zeile stünde die
// Test-Datenbank ohne Schema da und jeder Zugriff liefe gegen leere Tabellen. Mit ihr läuft in
// Tests dieselbe Migrationskette wie im Betrieb — und weil seit dem Wegfall von Hibernate niemand
// mehr beim Kontextstart prüft, ob Schema und Persistenzmodell zusammenpassen, ist das hier die
// Stelle, an der eine vergessene Migration auffliegt.
