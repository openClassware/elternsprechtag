...
public interface BuchungRepository extends ListCrudRepository<Buchung, UUID> {
  ...
  /**
   * Die Buchungen zu den gegebenen Ids, chronologisch nach Startzeit — die Menge eines einzelnen
   * Eltern-Submits für die Bestätigungsmail. Unbekannte Ids liefern schlicht keine Zeile. Lädt
   * Termin samt Sprechtag und den Lehrauftrag (mit Lehrer/Klasse/Fach) per {@link EntityGraph} mit,
   * damit das Record-Mapping im Service unter {@code open-in-view=false} ohne Lazy-Zugriff auskommt.
   */
  @EntityGraph(
      attributePaths = {
        "termin",
  ...
  List<Buchung> findByIdInOrderByTermin_StartzeitAsc(Collection<UUID> ids);
}
